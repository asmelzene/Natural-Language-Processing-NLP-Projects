...
def seg_for_docs_in_a_file(input_filename, input_sep, content_idx,  output_fileanme, t_len=0, did_idx=None, label_idx=None, out_sep='\t',
    ...
    for line in open(input_filename):
        ...
        if did_idx != None and label_idx !=None:
            segged_file.write(did + out_sep  + label + out_sep+ liststr(term_list).encode('utf-8') + '\n')
        if label_idx != None and did_idx ==None:
            segged_file.write(label + out_sep+  liststr(term_list).encode('utf-8') + '\n')
        if label_idx == None and did_idx !=None:
            segged_file.write(did + out_sep+  liststr(term_list).encode('utf-8') + '\n')
    segged_file.close()
    

seg_for_docs_in_a_file('train_binary.txt', '\t', 1, 'segged_train_binary.txt', did_idx=0, label_idx=-1)
seg_for_docs_in_a_file('test_binary.txt', '\t', 1, 'segged_test_binary.txt', did_idx=0)
