...
valid_dids = get_valid_dids(['segged_test_binary.txt', 'segged_train_binary.txt'],0)


result_file = open('train_senpos_binary.txt','w')
for line in open('train_binary.txt'):
    line = line.strip()
    if len(line)<3:continue
    [did, content, label] = line.split('\t')
    if not did in valid_dids:continue
    if cmp(label,'中性') ==0:continue
    ...
    for i in range(0, len(cutted_sen)):
        ...
        result_file.write(did + '\t' + str(spvec) + '\t' + label + '\t' + liststr(term_list).encode('utf-8') + '\n')
 
result_file = open('test_senpos_binary.txt','w')
for line in open('test_binary.txt'):
    line = line.strip()
    if len(line)<3:continue
    ...
    for i in range(0, len(cutted_sen)):
        ...
        term_list = [term for term in seg_list if len(term) > 0]
        if len(term_list)<2:continue
        result_file.write(did + '\t' + str(spvec)  + '\t' + liststr(term_list).encode('utf-8') + '\n')    