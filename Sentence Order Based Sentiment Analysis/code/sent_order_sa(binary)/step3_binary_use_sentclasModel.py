...
train_textX, train_dids, train_labels = get_textX_and_y_seggedFile('segged_train_binary.txt','\t',-1,did_idx=0,label_idx=1)
...
test_textX, test_dids, test_labels = get_textX_and_y_seggedFile('segged_test_binary.txt','\t',-1,did_idx=0)
pred = sentclas_clf.predict(test_textX)

pred_prob = sentclas_clf.predict_proba(test_textX)
i=0
result_file= open('segged_test_sentpred_binary.txt','w')
for line in open('segged_test_binary.txt'):
    line = line.strip()
    if len(line)<2:continue
    result_file.write(line + '\t' + str(pred[i])+'\t' + str(pred_prob[i]) + '\n')
    i = i+1

train_sen_textX, train_sen_dids, train_sen_labels = get_textX_and_y_seggedFile('train_senpos_binary.txt','\t',-1,did_idx=0)

pred = sentclas_clf.predict(train_sen_textX)
pred_prob = sentclas_clf.predict_proba(train_sen_textX)
i=0
result_file= open('train_senpos_sentpred_binary.txt','w')
for line in open('train_senpos_binary.txt'):
    line= line.strip()
    if len(line)<2:continue
    result_file.write(line + '\t' + str(pred[i])+'\t' + str(pred_prob[i]) + '\n')
    i = i+1
    
test_sen_textX, test_sen_dids, test_sen_labels = get_textX_and_y_seggedFile('test_senpos_binary.txt','\t',-1,did_idx=0)

pred = sentclas_clf.predict(test_sen_textX)
pred_prob = sentclas_clf.predict_proba(test_sen_textX)
i=-1
result_file= open('test_senpos_sentpred_binary.txt','w')
for line in open('test_senpos_binary.txt'):
    i = i+1
    line= line.strip()
    if len(line)<2:continue
    result_file.write(line + '\t' + str(pred[i])+'\t' + str(pred_prob[i]) + '\n')